...
public class AccountDataGatewayTest {
  ...
  private TestScenarioSupport testScenarioSupport =
      new TestScenarioSupport("tracker_registration_test");
  ...
  public void testCreate() {
    ...
    Map<String, Object> persisted =
        template.queryForMap("SELECT * FROM accounts WHERE id = ?", created.id);
  ...
  public void testFindBy() {
    ...
    assertThat(result)
        .containsExactly(accountRecordBuilder().id(1L).ownerId(12L).name("anAccount").build());
  }
}

...
public class AllocationControllerTest {
  ...
  private AllocationController allocationsController =
      new AllocationController(allocationDataGateway, client);

  @Test
  public void testCreate() {
    AllocationRecord record =
        testAllocationRecordBuilder()
            .id(20L)
            .projectId(31L)
    ...
    AllocationForm form = testAllocationFormBuilder().projectId(31L).firstDay("2016-02-20").build();
    ResponseEntity<AllocationInfo> response = allocationsController.create(form);

    verify(allocationDataGateway)
        .create(
            testAllocationFieldsBuilder()
                .projectId(31L)
                .firstDay(LocalDate.parse("2016-02-20"))
                .build());
    ...
    assertThat(response.getBody())
        .isEqualTo(
            testAllocationInfoBuilder().id(20L).projectId(31L).firstDay("2016-02-20").build());
  }

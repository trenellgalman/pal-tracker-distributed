...
public class AllocationDataGatewayTest {
  ...
  private final TestScenarioSupport testScenarioSupport =
  ...
  private final AllocationDataGateway gateway =
      new AllocationDataGateway(testScenarioSupport.template);
  ...
  public void testCreate() {
    ...
    Map<String, Object> persisted =
        template.queryForMap("select * from allocations WHERE id = ?", created.id);
  ...
  public void testFindAllByProjectId() {
    template.execute(
        "insert into allocations (id, project_id, user_id, first_day, last_day) values (97336, 22, 12, '2016-01-13', '2016-09-17')");

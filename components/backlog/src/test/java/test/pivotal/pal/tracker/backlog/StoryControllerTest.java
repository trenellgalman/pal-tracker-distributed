...
public class StoryControllerTest {
  ...
  public void testCreate() {
    StoryRecord record = storyRecordBuilder().id(4L).projectId(3L).name("Something Fun").build();

    doReturn(record)
        .when(storyDataGateway)
        .create(storyFieldsBuilder().projectId(3L).name("Something Fun").build());

    doReturn(new ProjectInfo(true)).when(client).getProject(anyLong());

    StoryForm form = storyFormBuilder().projectId(3L).name("Something Fun").build();
  ...
  public void testFailedCreate() {
    ...
    StoryForm form = testStoryFormBuilder().projectId(3L).build();
  ...
  public void testList() {
    List<StoryRecord> records =
        asList(testStoryRecordBuilder().id(12L).build(), testStoryRecordBuilder().id(13L).build());
    ...
    assertThat(result)
        .containsExactlyInAnyOrder(
            testStoryInfoBuilder().id(12L).build(), testStoryInfoBuilder().id(13L).build());
  }
}

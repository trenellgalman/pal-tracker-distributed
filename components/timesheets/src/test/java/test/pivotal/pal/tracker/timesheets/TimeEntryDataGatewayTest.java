...
public class TimeEntryDataGatewayTest {
  ...
  private TestScenarioSupport testScenarioSupport =
      new TestScenarioSupport("tracker_timesheets_test");
  ...
  public void testCreate() {
    ...
    Map<String, Object> persisted =
        template.queryForMap("SELECT * FROM time_entries WHERE id = ?", created.id);
  ...
  public void testFindAllByUserId() {
    template.execute(
        "insert into time_entries (id, project_id, user_id, date, hours) values (2346, 22, 12, '2016-01-13', 8)");
